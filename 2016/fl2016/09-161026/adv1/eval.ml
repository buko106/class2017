...
let rec eval_expr env e =
  ...
  | EVar x ->
     (try
        let d = lookup x env in
        value_with_update (x,d)
      with
      | Unbound -> raise (EvalErr ("Unbound variable "^x)))
  | EAdd (e1,e2) ->
  ...
  | ELet ( xe, e) ->
     eval_expr (extend_rec (List.map (fun (x,e)->(x,ref (DThunk (e,ref env)))) xe)env ) e
  | ELetRec ( fxes ,e ) ->
     let oenv = ref [] in
     (oenv := extend_rec (List.map (fun (f,x,e) -> (f, ref (DThunk(EFun(x,e),oenv)))) fxes) env ;
      eval_expr !oenv e)
  | EFun ( x , e) -> VFun ( x , e , env)
  | EApp ( f , e) -> 
     (match eval_expr env f  with
      | VFun ( x , body , fun_env ) -> eval_expr (extend x (ref (DThunk (e,ref env))) fun_env) body
  ...
  | EMatch ( e , pes ) ->
     ...
     | (xvs,e')::_ -> eval_expr (extend_rec (List.map (fun (x,v)-> (x,ref (DVal v))) xvs) env) e'
     )
  ...
  | ECons ( e1 , e2 ) -> 
     ...
     let v2 = eval_expr env e2 in
     VCons ( v1 , v2 )
  | _ -> raise (EvalErr "Not Implemented")
and value_with_update : name * dval ref -> value = fun (x,d) ->
  match !d with
  | DVal v -> v
  | DThunk (e,env') ->
     let v = eval_expr !env' e in
     let _ = d := DVal v in
     let _ = if !trace then
               (print_string ("bind "^x^"=" );print_value v;print_newline())
             else 
               () in v
...
let rec eval_topleveldec xe id_v env = (* eval for let..and..and *)
  ...
  |(x,e)::tl-> eval_topleveldec tl ((x, ref (DThunk (e,ref env)))::id_v) env
...
let eval_command env tyenv c =
  ...
  | CDecl xe_s ->
     ...
     (List.map (fun (x,d)->(x, value_with_update (x,d))) msgs,newenv,newtyenv)
  | CRecDecl fxes ->
     ...
     let oenv = ref [] in
     let (msgs,newenv) = (oenv := (List.map (fun (f,x,e) -> (f,ref (DThunk(EFun(x,e),oenv)))) fxes)@env ;( List.map (fun (f,_,_) -> ("val "^f^" : "^string_of_ty (lookup f newtyenv),eval_expr !oenv (EVar f))) fxes ,!oenv)) in (List.rev msgs,newenv,newtyenv)
  | CNil  -> ( [] , env , tyenv )
  | CTrace flag -> trace := flag ; ( [] , env , tyenv )
  | CQuit -> raise Exit
