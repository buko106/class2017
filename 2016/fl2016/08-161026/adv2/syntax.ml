...
let rec print_value v = if true then () else
  match v with
  | VInt i  -> print_int i
...
let rec string_of_ty ((forall_list,ty): type_schema) = 
  ...
  let rec list_up_tvar l1 ty = 
    ...
    | TVar i -> insert l1 i
    | TSamePair t1 -> list_up_tvar l1 t1
  in
  ...
  let rec string_of_ty_sub ty paren =
    ...
    | TSamePair t ->  (if false then "" else "(")^string_of_ty_sub t 29 ^ " samepair" ^ (if false then "" else ")")
