...
let rec print_value v =
  ...
  | VFun (_,_,_) | VMakeRef -> print_string "<fun>"
  ...
  | VUnit -> print_string "()"
  | VRef r -> (print_string "{contents = ";print_value !r ;print_string "}")
...
let rec string_of_ty ((forall_list,ty): type_schema) =
  ...
  let rec string_of_ty_sub ty paren =
    ...
    | TRef t ->  (if paren < 30 then "" else "(")^string_of_ty_sub t 29 ^ " ref" ^ (if paren < 30 then "" else ")")
    | TVar i ->
