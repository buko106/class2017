...
let rec eval_expr env e =
  ...
  | EApp ( f , e) -> 
     (match eval_expr env f  with
      ...
      | VMakeRef -> VRef (ref (eval_expr env e))
  ...
  | EMatch ( e , pes ) ->
     ...
     | (xvs,e')::_ -> eval_expr (extend_rec xvs env) e')
  ...
  | ECons ( e1 , e2 ) -> 
     ...
     VCons ( v1 , v2 )
  | ESeq ( e1 , e2 ) ->
     let _ = eval_expr env e1 in eval_expr env e2
  | EUnit -> VUnit
  | EAssign (e1,e2) -> (match eval_expr env e1 with
                        | VRef r -> let _ = r := (eval_expr env e2) in
                                    VUnit
                        | _ -> raise (EvalErr "ref value was expected in 'assign'"))
  | EGet e1 -> (match eval_expr env e1 with
                | VRef r -> !r
                | _ -> raise (EvalErr "ref value was expected in '!'"))
  | EWhile (e1,e2) -> let check_condition () = (match eval_expr env e1 with
                                             | VBool b -> b
                                             | _ -> raise (EvalErr "bool value was expected in while")) in
  ...
  | EFor (x,e1,e2,e3) ->
     ...
     | (VInt a,VInt b) -> for i=a to b do
                            eval_expr (extend x (VInt i) env) e3
                          done; VUnit
     | _ -> raise (EvalErr "int value was expected in 'for'")
