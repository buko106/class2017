...
let rec eval_expr env e =
  ...
  | ELet ( xe, e) ->
     eval_expr (extend_rec (List.map (fun (x,e)->(x,Thunk (e,ref env))) xe)env ) e
  | ELetRec ( fxes ,e ) ->
     let oenv = ref [] in
     (oenv := extend_rec (List.map (fun (f,x,e) -> (f, Thunk(EFun(x,e),oenv))) fxes) env ;
      eval_expr !oenv e)
  | EFun ( x , e) -> VFun ( x , e , env)
  | EApp ( f , e) -> 
     (match eval_expr env f  with
      | VFun ( x , body , fun_env ) -> eval_expr (extend x (Thunk (e,ref env)) fun_env) body
  ...
  (*    | (xvs,e')::_ -> eval_expr (extend_rec env (List.map (fun (x,v)-> (\*impossible*\)) xvs)) e') *)
...
let rec eval_topleveldec xe id_v env = (* eval for let..and..and *)
  ...
  |(x,e)::tl-> eval_topleveldec tl ((x,Thunk (e,ref env))::id_v) env
...
let eval_command env tyenv c =
  ...
  | CDecl xe_s ->
     ...
     (List.map (fun (x,Thunk(e,env))->(x,eval_expr !env e)) msgs,newenv,newtyenv)
  | CRecDecl fxes ->
     ...
     let oenv = ref [] in
     let (msgs,newenv) = (oenv := (List.map (fun (f,x,e) -> (f,Thunk(EFun(x,e),oenv))) fxes)@env ;( List.map (fun (f,_,_) -> ("val "^f^" : "^string_of_ty (lookup f newtyenv),eval_expr !oenv (EVar f))) fxes ,!oenv)) in (List.rev msgs,newenv,newtyenv)
  | CNil  -> ( [] , env , tyenv )
