...
uint64_t genetic_play( vector<uint8_t> genome , const uint64_t player , const uint64_t opponent ){
  for( int i = 0 ; i < 80 ; ++i){
    pos_val[i] = ((int)genome[i]) - OFFSET ;
  }
  for( int i = 0 ; i < 80 ; ++i){
    move_val[i]= ((int)genome[80+i]) - OFFSET ;
  }
