...
let rec ty_subst : subst -> ty -> ty = fun subst ty ->
  ...
  | TSamePair t1-> TSamePair(ty_subst subst t1)
...
let rec subst_rec : tyvar -> ty -> ty -> ty = fun a t ty -> (* ty[a:=t] *)
  ...
  |TSamePair t1 -> TSamePair (subst_rec a t t1)
